...
bool StartScene::init()
{
	...
	//add label
	auto label_showmsg = Label::create("Who Can Hold On 50sec", "verdana", 20, Size::ZERO, TextHAlignment::LEFT, TextVAlignment::TOP);
	...
	auto label_start = Label::create("StartGame", "verdana", 20, Size::ZERO, TextHAlignment::LEFT, TextVAlignment::TOP);
	...
	menuend->setPosition(Vec2(visibleSize.width / 2,visibleSize.height/2 - label_end->getContentSize().height*2));
	this->addChild(menuend);
	label_end->setPosition(Vec2(visibleSize.width / 2, visibleSize.height / 2 - label_end->getContentSize().height * 2));
	return true;
}
